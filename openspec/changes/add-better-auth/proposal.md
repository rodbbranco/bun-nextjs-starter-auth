...
- `route-protection`: Middleware-based route protection — redirect unauthenticated users to `/login`
...
- **New files**: `/lib/auth/auth.ts`, `/lib/auth/auth-client.ts`, `/lib/auth/helpers.ts`, `/db/schema.ts`, `/db/index.ts`, `/drizzle.config.ts`, `middleware.ts`, `/app/login/page.tsx`, `/app/dashboard/page.tsx`, `.env.local.example`
